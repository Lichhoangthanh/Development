...
# def show(course_id):
#     course = Course.query.get(course_id)
#     reviews = Review.query.filter_by(course_id=course_id).order_by(Review.created_at.desc()).limit(5).all()
...
#     else:
#         sorted_reviews = reviews  # Вариант по умолчанию: не выполнять сортировку

#     return sorted_reviews

# def filter_reviews(sort_order, course_id):
#     if sort_order == 'newest':
...
#     elif sort_order == 'negative':
#         reviews = Review.query.filter_by(course_id=course_id).order_by(Review.rating).all()
#     else:
...
#     # Получение идентификатора курса из запроса
#     course_id = request.args.get('course_id')

#     # Получение отфильтрованных отзывов
#     reviews_otz = filter_reviews(sort_order, course_id)

#     # Получение информации о текущем курсе
#     course = Course.query.get(course_id)

#     reviews_all = Review.query.filter_by(course_id=course_id).order_by(Review.created_at.desc()).all()
    

#     # Отображение отзывов в шаблоне
#     return render_template('courses/reviews.html', reviews_all=reviews_all, sort_order=sort_order, course=course,  reviews_otz=reviews_otz)
...
#     # Получение отфильтрованных отзывов
#     reviews = filter_reviews(sort_order, course_id)

#     # Получение информации о текущем курсе
#     course = Course.query.get(course_id)

#     # Отображение отзывов в шаблоне
#     return render_template('courses/reviews.html', reviews=reviews, sort_order=sort_order, course=course)
